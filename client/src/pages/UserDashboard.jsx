...
const StrengthBar = ({ password }) => {
  ...
  return (
    <div className='mt-2'>
      ...
      <p className={`text-[10px] uppercase font-bold tracking-wider mt-1 ${idx<2?'text-red-500':idx<4?'text-amber-400':'text-amber-500'}`}>{labels[idx]}</p>
    </div>
  )
}
        ...
        return (
          <div key={step} className='flex items-center flex-1'>
            <div className='flex flex-col items-center w-full'>
              <div className={`w-6 h-6 rounded-full flex items-center justify-center text-[10px] font-goldman font-bold transition-all duration-300 ${done ? 'bg-amber-500 text-slate-950 font-black shadow-md shadow-amber-500/20' : 'bg-slate-900 text-slate-400 border border-slate-800'} ${active ? 'ring-2 ring-offset-2 ring-offset-slate-950 ring-amber-500 scale-110' : ''}`}>
                {done && !active ? '✓' : i+1}
              </div>
              <span className={`text-[10px] uppercase tracking-wider mt-1.5 text-center leading-none font-goldman ${done ? 'text-amber-500/90 font-bold' : 'text-slate-400'}`}>{step}</span>
            </div>
            {i < steps.length-1 && <div className={`h-0.5 flex-1 mb-6 ${i < curIdx && !rejected ? 'bg-amber-500' : 'bg-slate-800'} transition-all duration-300`} />}
          </div>
        )
      })}
...
function UserDashboard() {
  ...
  const inputCls = 'w-full px-4 py-2.5 bg-white border border-slate-200 focus:border-amber-500 rounded-xl text-sm text-slate-800 focus:outline-none focus:ring-2 focus:ring-amber-500/15 transition-all'
  const labelCls = 'block text-[10px] uppercase tracking-wider font-bold text-slate-500 mb-1.5'
  const tabCls   = (t) => `px-5 py-3 text-sm font-goldman font-bold uppercase tracking-widest transition-all duration-300 cursor-pointer border-b-2 ${activeTab===t ? 'border-amber-500 text-amber-600' : 'border-transparent text-slate-500 hover:text-amber-500/80'}`

  if (loading) return (
    <div className='min-h-screen bg-slate-50 flex items-center justify-center'>
      <div className='flex gap-1.5'>
        {[0,1,2].map(i=><div key={i} className='w-3 h-3 bg-amber-500 rounded-full animate-bounce' style={{animationDelay:`${i*100}ms`}}/>)}
      </div>
    </div>
  )
  ...
  return (
    <div className='min-h-screen bg-slate-50 text-slate-800 font-roboto animate-fade-in pb-16'>
      ...
      <div className='bg-white/85 backdrop-blur-md border-b border-slate-200 px-6 py-4 flex items-center justify-between sticky top-0 z-30 shadow-sm'>
        ...
        <div className='flex items-center gap-5'>
          ...
          <button onClick={() => { logout(); navigate('/') }}
            className='text-xs px-4 py-1.5 border border-slate-200 text-slate-700 rounded-xl hover:bg-amber-500 hover:text-white hover:border-amber-500 font-goldman font-bold uppercase tracking-wider cursor-pointer transition-all duration-350'>
            {t('logout')}
          </button>
        </div>
      </div>
      ...
      <div className='max-w-4xl mx-auto px-4 py-12'>
        <h1 className='text-3xl sm:text-4xl font-goldman font-bold text-amber-500 uppercase tracking-widest mb-8'>{t('dashboard')}</h1>

        {/* Email verification banner */}
        {user && !user.email_verified && (
          <div className='mb-8 bg-gradient-to-r from-amber-500/10 to-transparent border border-amber-500/30 rounded-2xl p-6 shadow-lg animate-pulse-slow'>
            {!showVerify ? (
              <div className='flex flex-col sm:flex-row sm:items-center justify-between gap-4'>
                <div className='flex items-start gap-4'>
                  <span className='text-amber-500 text-2xl mt-0.5'>✉️</span>
                  <div>
                    <p className='font-goldman font-bold text-sm text-amber-500 uppercase tracking-wider'>Verify your email address</p>
                    <p className='text-xs text-slate-600 mt-1 font-light leading-relaxed'>Your email <strong className='font-mono font-normal text-amber-600'>{user.email}</strong> is not verified yet.</p>
                  </div>
                </div>
                <button onClick={handleSendVerification}
                  className='px-5 py-2 bg-amber-500 hover:bg-amber-400 text-white text-xs font-goldman font-bold uppercase tracking-widest rounded-xl transition-all duration-300 shadow-md hover:scale-105 active:scale-95 cursor-pointer self-start sm:self-auto'>
                  Verify Now
                </button>
              </div>
            ) : (
              <form onSubmit={handleVerifyEmail} className='space-y-4'>
                <p className='text-sm font-goldman font-bold text-amber-500 uppercase tracking-wider'>Enter the 6-digit code sent to {user.email}</p>
                  ...
                  <div className='flex gap-2'>
                    {verifyOtp.map((d, i) => (
                      <input key={i} ref={el => verifyRefs.current[i] = el}
                        ...
                        className='w-10 h-10 text-center text-lg font-bold border border-slate-200 rounded-lg bg-white text-slate-800 focus:outline-none focus:border-amber-500' />
                    ))}
                  </div>
                  <button type='submit' disabled={verifying}
                    className='px-5 py-2.5 bg-amber-500 hover:bg-amber-400 text-white text-xs font-goldman font-bold uppercase tracking-widest rounded-xl transition-all duration-300 shadow-md hover:scale-105 active:scale-95 cursor-pointer'>
                    {verifying ? '…' : 'Confirm'}
                  </button>
          ...
          <div className='space-y-6'>
            {complaints.length === 0 ? (
              <div className='bg-white border border-slate-200 rounded-2xl p-12 flex flex-col items-center text-slate-400'>
                <ComplaintIcon className='w-14 h-14 mb-4 opacity-30 text-amber-500' />
                <p className='font-goldman font-bold text-lg uppercase tracking-wide text-slate-600'>{t('no_complaints')}</p>
                <Link to='/compliants' className='mt-6 px-6 py-3 bg-amber-500 hover:bg-amber-400 text-white font-goldman font-bold uppercase tracking-widest rounded-xl transition-all duration-300 shadow-md hover:scale-105 active:scale-95'>
                  {t('submit_one')}
                </Link>
              </div>
            ) : complaints.map(c => (
              <div key={c.id} className='bg-white border border-slate-200 rounded-2xl p-6 shadow-md relative overflow-hidden transition-all duration-300 hover:border-amber-500/30 group'>
                <div className='flex items-start justify-between mb-3.5'>
                  <div>
                    <p className='font-goldman font-bold text-lg text-slate-800 tracking-wide group-hover:text-amber-600 transition-colors'>{c.type || '—'}</p>
        ...
        {activeTab === 'applications' && (
          <div className='space-y-6'>
            {applications.length === 0 ? (
              <div className='bg-white border border-slate-200 rounded-2xl p-12 flex flex-col items-center text-slate-400'>
                <FileIcon className='w-14 h-14 mb-4 opacity-30 text-amber-500' />
                <p className='font-goldman font-bold text-lg uppercase tracking-wide text-slate-600'>{t('no_apps')}</p>
                <Link to='/vaccancy' className='mt-6 px-6 py-3 bg-amber-500 hover:bg-amber-400 text-white font-goldman font-bold uppercase tracking-widest rounded-xl transition-all duration-300 shadow-md hover:scale-105 active:scale-95'>
                  {t('apply_now')}
                </Link>
              </div>
            ) : applications.map(a => (
              <div key={a.id} className='bg-white border border-slate-200 rounded-2xl p-6 shadow-md relative overflow-hidden transition-all duration-300 hover:border-amber-500/30 group'>
                <div className='flex items-start justify-between mb-3.5'>
                  <div>
                    <p className='font-goldman font-bold text-lg text-slate-800 tracking-wide group-hover:text-amber-600 transition-colors'>{a.vacancy_title || '—'}</p>
                    <p className='text-xs text-amber-600 font-goldman uppercase tracking-wider mt-1.5'>{a.location} · {a.job_type} · {a.category}</p>
                ...
                {a.cv_path && (
                  <a href={a.cv_path} target='_blank' rel='noreferrer'
                    className='inline-flex items-center gap-1.5 mt-4 text-xs font-goldman font-bold uppercase tracking-wider text-amber-600 hover:text-amber-500 hover:underline transition-all duration-300'>
                    <FileIcon className='w-3.5 h-3.5 text-amber-500' /> View submitted CV
                  </a>
                )}
              </div>
            ))}
          ...
          <div className='space-y-6 max-w-lg animate-fade-in'>
            ...
            <div className='bg-white border border-slate-200 rounded-2xl shadow-md overflow-hidden'>
              <div className='flex items-center justify-between px-6 py-4.5 bg-slate-50 border-b border-slate-200'>
                ...
                <button onClick={() => setEditingProfile(!editingProfile)} className='p-2 rounded-lg bg-slate-100 text-slate-500 hover:text-amber-600 hover:bg-slate-200 cursor-pointer transition-all duration-300'>
                  ...
                  <div key={key} className='flex justify-between items-center py-4'>
                    ...
                    {editingProfile
                      ? <input type='text' value={profileForm[key]} onChange={e=>setProfileForm(p=>({...p,[key]:e.target.value}))}
                          className='w-52 px-3 py-1.5 bg-white border border-slate-200 focus:border-amber-500 rounded-xl text-sm text-right text-slate-800 focus:outline-none focus:ring-2 focus:ring-amber-500/15 transition-all' />
              ...
              {editingProfile && (
                <div className='px-6 py-4 border-t border-slate-100 bg-slate-50/50 flex justify-end gap-3'>
                  ...
                  <LoadingButton isLoading={isSaving} onClick={handleSaveProfile}
                    className='px-5 py-2.5 bg-amber-500 hover:bg-amber-400 text-white text-xs font-goldman font-bold uppercase tracking-widest rounded-xl transition-all duration-300 cursor-pointer shadow-md'>
                    {t('save')}
                  </LoadingButton>
                </div>
              )}
            ...
            <div className='bg-white border border-slate-200 rounded-2xl shadow-md overflow-hidden'>
              <div className='flex items-center justify-between px-6 py-4.5 bg-slate-50 border-b border-slate-200'>
                ...
                <button onClick={() => setEditingPass(!editingPass)} className='p-2 rounded-lg bg-slate-100 text-slate-500 hover:text-amber-600 hover:bg-slate-200 cursor-pointer transition-all duration-300'>
                  <EditIcon className='w-4 h-4' />
                </button>
              </div>
              {editingPass ? (
                  ...
                  <div>
                    <div className='flex justify-between items-center mb-1.5'>
                      ...
                      <button type='button' onClick={() => { const p=generateStrongPassword(); setPassForm(prev=>({...prev,newPassword:p,confirmPassword:p})) }}
                        className='text-[10px] text-amber-600 hover:text-amber-500 font-goldman font-bold uppercase tracking-wider cursor-pointer transition-all'>{t('suggest')}</button>
                    </div>
